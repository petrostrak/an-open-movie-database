...
// Implement a MarshalJSON() on the Runtime type so that it satisfies the
// json.Marshaler interface. This should return the JSON-encoded value for
// movie runtime.
...
func (r *Runtime) UnmarshalJSON(jsonValue []byte) error {
	...
	// double-quotes from the string. If we cannot unquote it, then we return the
	...
	// Sanity check the parts of the string to kame sure it was in the
	// expected format. If it isn't, we return the ErrInvalidRuntimeFormat again.
	...
	// Note that we use the * operator to deference the receiver (which is a
	// pointer to a Runtime type) in order to set the underlying value of the pointer.
	*r = Runtime(i)
